...
initialDronesPerGS = 5;
requestInterval = 2;
chargingTime = 10;
simDuration = 180;
droneSpeed = 1;
verticalSpeed = 10;
boxSize = 50; % Box size for visualization
GS_zLevel = 3;
slotCount = 3;
slotHeightStep = 2;
...
%% Fixed GS Positioning based on the provided graph image
% Scaling positions to fit within a reasonable simulation space
% Node positions based on the graph layout (adjusted for boxSize)
GS_positions = [
...
%% Display Fixed Node Positions
fprintf('\n--- Fixed Node Positions ---\n');
for i = 1:numGS
    fprintf('Node %d: (%.1f, %.1f)\n', i-1, GS_positions(i,1), GS_positions(i,2));
end
