...
%% PARAMETERS
numGS = 20;                    % Number of ground stations (parameter)
areaSize = 100;              % Area size (1000x1000) - parameter
gsRange = 40;                % Communication range for each GS (parameter)
totalDR = 60;                 % Total number of drones available (parameter)
minDistGS = 20;               % Minimum distance between ground stations (parameter)
requestInterval = 2;
chargingTime = 15;            % Charging time = 15 sec (parameter)
simDuration = 120;
droneSpeed = 1.0;
GS_zLevel = 0;
flightZ = 2;
...
for t=0:simDuration
    ...
    if mod(t, requestInterval) == 0 && dronesUsed < totalDR
        ...
        drone.path = path;
        drone.state = "charging";  % Start with charging for 15 seconds
        drone.hop = 1;
        drone.timeInState = 0;
        ...
        fprintf('Time %d: Request #%d - Drone spawned at GS %d to GS %d with path: %s (Drones used: %d/%d)\n', ...
                t, requestCounter, src, dest, mat2str(drone.path), dronesUsed, totalDR);
    ...
    for i = 1:length(activeDrones)
        ...
        if drone.hop < length(drone.path)
            nextGS = drone.path(drone.hop + 1);
            str = sprintf('DR-%d: %s at GS %d -> GS %d', i, drone.state, currentGS, nextGS);
        else
            str = sprintf('DR-%d: %s at GS %d (destination)', i, drone.state, currentGS);
        end
