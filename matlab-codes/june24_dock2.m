...
poissonNumIntervals = 5;    % Number of intervals for requests
poissonIntervalLength = 10;  % Length of each interval in seconds (e.g., 5 seconds)
poissonAvgRatePerInterval = 5;
...
totalCollisions = 0;      % Counter for total collisions
...
droneTemplate = struct(...
    ...
    'actualFlyTime', 0, ...
    'actualTotalTime', 0, ...
    'collisions', struct('time', {}, 'otherDroneID', {}) ... % NEW: Collision log
);
    ...
    if numActiveDrones > 1
        for i = 1:numActiveDrones
            for j = i+1:numActiveDrones % Check each unique pair
                ...
                % Only check for collisions if both drones are moving
                % (or if they are at the same GS, but then collision shouldn't happen due to queuing)
                % The problem states "on the path", implying movement.
                if strcmp(drone1.state, "moving") && strcmp(drone2.state, "moving")
                    
                    % Check Z-level first
                    if abs(drone1.pos(3) - drone2.pos(3)) < zTolerance
                        % If Z-levels are approximately the same, check 2D distance
                        dist2D = norm(drone1.pos(1:2) - drone2.pos(1:2));
                        if dist2D < collisionThreshold
                            % Collision detected!
                            totalCollisions = totalCollisions + 1;
                            fprintf('Time %d: COLLISION DETECTED between Drone-%d and Drone-%d at (%.2f,%.2f,%.2f) and (%.2f,%.2f,%.2f)!\n', ...
                                    t, drone1.id, drone2.id, drone1.pos(1), drone1.pos(2), drone1.pos(3), ...
                                    drone2.pos(1), drone2.pos(2), drone2.pos(3));
                            
                            % Log collision in both drones' records
                            % Check if collision already logged for this exact time and pair to avoid duplicates
                            alreadyLogged1 = false;
                            for k = 1:length(drone1.collisions)
                                if drone1.collisions(k).time == t && drone1.collisions(k).otherDroneID == drone2.id
                                    alreadyLogged1 = true;
                                    break;
                                end
                            end
                            if ~alreadyLogged1
                                if isempty(drone1.collisions) % Initialize if first collision
                                    drone1.collisions = struct('time', t, 'otherDroneID', drone2.id);
                                else
                                    drone1.collisions(end+1) = struct('time', t, 'otherDroneID', drone2.id);
                                end
                            end

                            alreadyLogged2 = false;
                            for k = 1:length(drone2.collisions)
                                if drone2.collisions(k).time == t && drone2.collisions(k).otherDroneID == drone1.id
                                    alreadyLogged2 = true;
                                    break;
                                end
                            end
                            if ~alreadyLogged2
                                if isempty(drone2.collisions) % Initialize if first collision
                                    drone2.collisions = struct('time', t, 'otherDroneID', drone1.id);
                                else
                                    drone2.collisions(end+1) = struct('time', t, 'otherDroneID', drone1.id);
                                end
                            end
                            
                            % Update activeDrones array with modified drone structs
                            activeDrones(i) = drone1;
                            activeDrones(j) = drone2;
...
fprintf('DR ID\tWait Time (s)\tCharge Time (s)\tFly Time (s)\tTotal Time (s)\tCollisions\n'); % NEW: Added Collisions column
fprintf('-----\t-------------\t---------------\t------------\t--------------\t----------\n');
totalQueueTime = 0;
totalChargingTime = 0;
totalFlyTime = 0;
totalJourneyTimeOverall = 0;
totalCollisionsLogged = 0; % NEW: To sum up individual drone collision logs
for i = 1:length(allDrones)
    ...
    % Note: A single collision event is logged twice (once for each drone),
    % so totalCollisionsLogged will be double the actual collision count.
    % The 'totalCollisions' variable keeps the true count.
...
% NEW: Print total number of collisions from the simulation-wide counter
...
end % End of ensureConnectivity function